...
## 输入工件
- Collie review: features/<active>/reviews/review-<节点>-Collie-YYYYMMDD-HHMM.md
- Teddy review: features/<active>/reviews/review-<节点>-Teddy-YYYYMMDD-HHMM.md
- Husky response: features/<active>/reviews/response-<节点>-Husky-YYYYMMDD-HHMM.md
...
```json
{
  ...
  "record_path": "features/<active>/reviews/resolution-<节点>-YYYYMMDD-HHMM.md",
  "key_findings": [
    "关键发现 1",
    "关键发现 2"
  ]
...
| `docs/agent-configs/templates/resolution-template.md` | Resolution 模板；承担早期方案中的 `review-resolution-template.md` 职责，避免重复模板漂移 | P0 |
...
- `resolution-<节点>-YYYYMMDD-HHMM.md` 是交回 router 的聚合 review record。
...
router -> canonical hf-*review/gate
  ...
  -> router consumes HF Verdict from resolution
```
...
```json
{
  ...
  "expected_record_path": "features/<active>/reviews/review-<节点>-<Agent>-YYYYMMDD-HHMM.md",
...
2. 根据参与矩阵判断 Collie / Teddy / Husky 是否必须参与。
3. 生成 Review Briefing。
4. 给出 Collie review request。
5. 给出 Teddy review request，若串行互读启用，则带入 Collie review 路径。
6. 收集 Husky feasibility response。
7. 使用 resolution template 汇总。
8. 输出兼容 `reviewer-return-contract.md` 的 HF Verdict。
...
- `resolution-<节点>-YYYYMMDD-HHMM.md` 是交回 HF router 的唯一多 Agent 汇总输入。
...
- Collie review 必须包含角色锚定检查、反方立场、分歧发现量化。
...
- Teddy review 必须包含角色锚定检查、反方立场、分歧发现量化。
