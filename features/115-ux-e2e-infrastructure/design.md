...
| 阶段 | TBD 阈值（修订建议） | 当前实际 |
|------|--------------------|---------|
| F115 finalize（结构完整） | **L2 列** `<TBD` ≤ 行数 × 0.4 = 3.6（即 L2 列 ≥ 60% 有继承函数名） | ~~3 个（FR-1.2/1.7/1.8），达标~~ **T-14 实施修正：4 个（FR-1.2/1.5/1.7/1.8），4 > 3.6，未达 design 自定阈值** |
| F201 finalize（测试代码完整） | **全表** `<TBD` 计数 = 0 | 待 F201 |

> **T-14 实施回写**（2026-06-20）：design §4.3.3 原汇总"3 个 TBD，达标"算术错误——逐行分布显示 **FR-1.5 L2 也是 TBD**（探测**非目标站点**可达性，F201 全新增能力，无 F001~F004 继承测试可填；F001 测试只覆盖目标站点可达性）。修正后 L2 列 TBD = 4。
>
> **影响评估**：
> - **不影响 spec FR-2.3.1-R3a 阶段 1 通过**（spec 仅要求"占行 + 等级标注"，无 L2 TBD 阈值）
> - design §4.3.3 的"L2 ≤ 3.6"为**自定质量参考**而非 spec 硬验收；4/9 = 44% vs 40% 自定阈值，轻微超标
> - **不触发 spec 勘误**：spec 原文未被反驳，仅 design 自定阈值在算术修正后未达；待 F201 design 启动后这 4 个 L2 TBD 由 F201 新增 FR 验收测试填入
>
> **design 决策**：F115 finalize 时 L2 列 TBD = 4 作为已记录的 quality 警示（不阻塞），其它列 TBD 推到 F201 design 阶段填入。
