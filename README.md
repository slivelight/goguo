...
- **Feature 115: UX E2E 自动化测试基础设施正式化** — `closed`（2026-06-21 workflow closeout）
  - 26/27 任务完成（M3~M8 全部收口，T-25 按 C-P4 隔离推迟到 F201 design 阶段）
  - F114 PoC 升级：均值 95s → 33s（SC-2 PASS）；flakiness 0/20 = 0%（SC-7 PASS）
  - 立项 §7 强制规范：所有新 Feature design.md 必须含 §N "L1~L5 自动化测试设计"章节
  - 详见 `features/115-ux-e2e-infrastructure/closeout.md`
...
- **Feature 201: 首次安装引导与基线确认闭环** — `spec 草稿`
  - 首个按 F115 §7 规范接入的 Feature（design 待启动将完成 §N 章节首案例演练）
  - 详见 `features/201-first-run-baseline-confirm/spec.md`
